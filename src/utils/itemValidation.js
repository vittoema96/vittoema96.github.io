...
/**
 * Validate if HP adjustment is within valid range
 * @param {number} currentHp - Current HP value
 * @param {number} adjustment - HP adjustment (positive or negative)
 * @param {number} maxHp - Maximum HP
 * @returns {Object} {isValid: boolean, newHp: number}
 */
export const validateHpAdjustment = (currentHp, adjustment, maxHp) => {
    const newHp = Math.max(0, Math.min(currentHp + adjustment, maxHp))
    return {
        isValid: true,
        newHp
    }
...
/**
 * Validate if caps adjustment is valid
 * @param {number} currentCaps - Current caps value
 * @param {number} adjustment - Caps adjustment (positive or negative)
 * @returns {Object} {isValid: boolean, newCaps: number, reason: string}
 */
export const validateCapsAdjustment = (currentCaps, adjustment) => {
    const newCaps = currentCaps + adjustment
